...
% introduzione al concetto di machine learning - fatto
% parlare delle diverse tipologie di trainigng (supervised, ecc) - fatto
% parlare di come è fatto un modello - fatto
% parlare delle varie tipologie di modelli per ML - fatto
Machine learning is a branch of Artificial Intelligence (AI) that deals with the creation and training of models that have the ability to learn from data. This technology is a growing field of data science that is gaining popularity thanks to its flexibility to adapt to many problems. Machine learning models can be trained for different purposes such as regression of unknown systems, classification of data, predictions of data behaviour and artificial generation of data. One of the most relevant fields in which ML has a huge impact is computer vision.\\
An ML models is a file generated from a training procedure with a specific setup. Training sessions are usually customized to make models learn specific tasks. The simplest ML model, called Deep Neural Network (DNN), is composed of neurons grouped inside layers that are connected in series as displayed in Figure \ref{fig:modelstructure}. The layers can be divided in: input layers, hidden layer and output layers. The input layer is the first of the model, and here the input data array is stored. Each neuron of this first layer contains a value from the input array or matrix that is fed to the model. The output layer is the last of the model. This contains the elaborated data which is usually an array of values. The hidden layers are all the layer in between the first and the last one. The types of hidden layer can change depending on the elaboration that is required for the application. Figure \ref{fig:modelstructure} shows a model with only fully connected layers. This type is characterized by the presence of neurons that are connected to every single neuron from the previous and next layer. Other types of layer can be: convolutional layer, which are able to apply a specific operation on images; max pooling layers, which are used for reducing the size of images maintaining the most relevant features; dropout layers.\\
...
An important role is played by the smallest element in a model structure, the neuron. This component is the one responsible for the evolution of the network and for the learning abilities of the model. A neuron, or node, can be seen mathematically as a function that takes many inputs and generates one single output. A shown in Figure \ref{fig:modelstructure}, one neuron takes all the inputs, multiplies each of them for its weight, sums them together and adds to the result a biased value. Then the result of the addition is fed through an activation function which is used for introducing a non linearity in the model. The type of activation function is usually defined depending on the application of the model. Some well known functions are Softax, ReLu, Sigmoid, Linear, and Hyperbolic Tangent. Note that in a fully connected layers, every single node receives as input the outcome from every single node of the previous layer. \\
The action of machine learning happens when a prediction is computed and a feedback is applied. An example of training step is the following. The model receives an input data sample, then it elaborates the sample by propagating it through all the layers. When the sample reaches the output layer it is transformed into a new array, which is dense of informations. In the case the model is used for classification, the prediction is compared with the true label, which is known since this is a training step. The error of the prediction is computed with a loss function which is then used for correcting the weights and biases from all the previous layers. If the training is performed on a big enough dataset, the model is then trained to recognize correctly data similar to the samples used for training.
ML trainings can be categorized in several groups depending on how the training is performed. The main groups are:

\begin{itemize}
\item Supervised learning: a type of training where the model is provided with input data samples and desired output. The model is trained to yield the same outcome as the one provided by using a feedback applied on the weights that is based on the prediction error. Supervised learning is commonly used in classification problems (when a model is trained to categorize data in groups) and in regression problems (when a model is trained to find the relation between variables).
\item Unsupervised learning: a type of training where the model is provided with only input data. During training the model learns to find patterns of the data and is able to categorize them in groups. This is commonly used in problems where the user is unsure about the properties of the dataset.
\item Reinforcement learning: a type of training where the model is provided with input data and rewards. The goal of the model is to elaborate the input data and comoute an output, depending on the rewards obtained the model learns from its mistakes and tries to maximize the reward that it will get. This type of training is commonly used for minimization of problems such as industrial automation (minimization of robot paths), data processing, and playing chess games.
\item Transfer learning: is not a real type of training, but more a technique that is used to transfer already gathered knowledge. This method takes an already trained model and fine-tunes just its last portion to train it on a new task that is still relevant to the application. 
\end{itemize}
...
Depending on the types of layer used inside a ML model different types of structures can be defined. Different types of structures can be used for specific applications. Models that are composed of only fully connected layers, also called Neural Netowrks (NN) are well suited for regression and classification of data. Layers known as convolutional layers, are wellwell suited for the elaboration of images. They are based on the use of a filter that moves over an image and perform the operation of convolution with the values coming from the pixels. Usually these types of layers compose the initial layer of a CNN model. These models use at first convolutional layers for the extraction of data, then flatten the matrix of manipulated data into an array which is then fed to a NN which perform the classification on the image.
Other types of models are the autoecnoders. These models can be seen as generative models since their role is to behave as constructor of lost data. An autoecnoder is composed of two groups of layers, the first is an encoder, which is a sequence of NN lauer with decreasing size, while the second is an encoder, which is a sequence of DNN layer with increasing size. The goal of an autoencoder is to take the input array, compress it into a smaller array and from there reconstruct the compressed array into its original array. If a trianing is done correctly, the decoder part of the model can be used for generating data from a very small quantity of data.\\
...
When deploying ML models on constrained device there are two main concerns, which are the available memory and the available computational power. To fight against these two limitations different techniques have been developed. Pruning and quantization are pre processing methods that can be employed on already trained model to reduce their memory footprint and the computation weight. \\
Pruning is a strategy that is adopted after the training of a model. This is required because at each step of a pruning procedure, the model of interest is de gradated and so its performances. In most cases the model developed for the application is bigger than actually required. This is due to the initial allocation of the desired structure and the following training of all the parameters inside it. The pruning method relies on the fact, that ina a model a certain number of parameters are actually redundant, non relevant or useless for the application. In fact the strategy idea is to completely remove the less important connection between neurons with the aim of reducing the memory required for storing the model and also reduce the amount of computations for an inference. 
A pruning step is composed of the following action: evaluate the importance of each weight based on their influence on the network output and performance, prune the less important weights (bring to 0 their value), re train the model that inevitably lost performance, check if the pruning is enough, if not perform another step. Figure \ref{fig:pruning} contains a block diagram showing the procedure of a pruning strategy.
...
An additional improvement brought by pruning is the reduced amount of computation. If done strategically, the pruning process can introduce sparsity (a localized population of zero values) in the matrices used for storing weights and biases. If the sparsity is injected with a structure it is then possible to also reduce the amount of computations, thus improving the efficiency of inference in terms of time, energy and complexity.\\

Another method that is often used for compressing ML models is quantization. Quantization is a process, usually applied after pruning, that aims at reducing the bits per weight. The idea is to reduce the memory dedicated to each weight for example FP32 (floating point 32), to a smaller amount of memory, such as INT8 (integer 8 bit). The conversion of weights from floating point to integer is an operation that can quickly and easily introduce errors  and make the model's accuracy drop. 
