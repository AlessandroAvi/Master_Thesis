...
% spiegareo cosa è CL
% spiegare uso, utilizzo, come mai serve CL, a cosa è utile
% spiegare i problemi che vengono introdotti, cosa deve aggirare
...
Applications of machine learning are based on the use of model for the elaboration of data. An important aspect regarding standard ML approaches is the possibility of these models to perform only inference over the inputs. This limits the scenarios in which these applications can be deployed. The models are not capable of adapting to changing context, which are very common and compose the vast majority of applications that concern the use of MCUs. To overcome this problem an additional aspect can be introduced into standard ML methods, which is continual learning. \\
Continual learning is a paradigm of machine learning where models are trained in real time with streams of data presented sequentially. The main concern of this technology is to introduce to these meodel the ability to adapt to changes and drift over the data and the context. It is often the case that ML models are trained in ideal scenarios that use dataset composed of static data. This is not an optimal situation because later, at deploy time, the model could encounter problems such as drop in accuracy, and sub optimal behaviour. The use of CL fixes the rigidity of ML models by introducing the ability to train ML model in real time over stream of data. \\
...
\begin{figure}[h!]
    ...
    \caption{Block diagram showing the steps of a pruning procedure. IMMAGINE NON DEFINITIVA}
    \label{fig:pruning}    
\end{figure}  
