...
%\usepackage{apacite}
\usepackage[backend = biber, backref = false, style=ieee]{biblatex}
...
Applications of machine learning are based on the use of trained models for the elaboration of data. One of the main aspect regarding application of ML models is their capability to perform only inference over input data. Most times, systems that rely solely on inferences are more than enough for the application purposes. But in other cases, the use of ML models with the aim of performing only inference is not enough. These scenarios are characterized by not static environments, which are subjected to continuous evolution and changes. In such scenarios the data that is recorded and fed to ML models is subject to drifts, which means that if a model is originally trained on a static dataset with similar properties to the initial state of the environment, it can quickly become obsolete. These models are not able to self-adapt to the context making the accuracy, performance and general reliability drop. Scenarios like these are very common in TinyML applications, mainly because tiny devices are usually deployed for extended periods of times, where the nature of recorded data can change quickly. To overcome the problem of obsolete ML models it is necessary to implement agents that make these models self-adjust and autonomous. A branch of ML well suited for this reason is Continual Learning (CL). \\
The main characteristic of CL is the possibility to take a pre-trained model and update it to adapt to new data. Most times these procedures are hard to perform because old data is quickly discarded or because there is no availability of storage (example: embedded devices memories have no room for data samples). The solution is to train models in real-time on streams of sequential data which is immediately discarded. The main aspect of CL is the implementation of autonomous agents that can control the data stream and the training abilities of the model. \\
Continual Learning is a paradigm of Machine Learning where models are strategically trained in real-time with streams of data presented sequentially. The main characteristic of CL is the development of agents and processes that can adapt to the situation, train models and increasingly improve their complexity and performances. These aspects makes CL models very suited for applications in dynamic scenarios, where the context data is subject to drift and evolution. To improve the flexibility of CL models another feature can be added to the ML models, the ability to recognize and adapt to new classes of data. This is an easy to implement element that directly improves the complexity of the model and its autonomous behaviour. Models that are equipped with this are able to recognize the presence of new classes and allocate new weights and biases dedicated to their inference. Systems structured in this way require for more works and more complex systems implementations, but allow for application that require minimum to no maintenance after deployment. \\
Paper \autocite{} proposes a clear distinction between types of data streams which directly defines also the types of CL systems that should be adopted for each case.
...
Figure \ref{fig:CLstrategies} contains a Venn Diagram showing the classification of the most common algorithms.
...
Another important paper about CL , this time not applied on MCUs is the the study \autocite{maltoni2019continuous}. Here the author's goal is to implement several algorithms and test them to evaluate their abilities to overcome catastrophic forgetting. The study applies already known strategy like LWF \autocite{li2017learning}, SI \autocite{zenke2017continual}, EWC \autocite{kirkpatrick2017overcoming} and it later proposes two custom made strategies called CWR+ and AR1. Note that CWR+ is an improvement of an already proposed algorithm developed in a previous paper \autocite{lomonaco2017core50} by the same authors. \\
An interesting application of CL strategies is proposed in paper \autocite{grau2021device}. A system composed of 2 microcontrollers and a central server is used for performing federated training. The idea is to perform training on the edge and frequently merge the models on the server. This allows for applications of CL in a distributed way (useful for IoT), where devices are used for performing fine tuning over different input streams, thus allowing the merged ML model to be composed from the best part of all ML models.
Another interesting study, not directly related to algorithms, is the one described in \autocite{sudharsan2021imbal}. Here the authors developed an easy to attach system, called Imbal-OL, that aims at balancing real-time streams of data. From their point of view, real life applications of CL strategies for IoT devices are characterized by unbalanced streams of data. This affects real-time trainings because models can easily overfit the inputs bringing to models with poor performances. The authors propose a small plug-in to be attached in between the raw input stream and the CL model that is able to apply pre processing on the data and make the input stream balanced. \\
One of the most relevant works for this thesis is the paper \autocite{ren2021tinyol}. Here the authors developed a small framework for an Arduino microcontroller that is able to apply continual learning (or on-line learning) strategies on an ML model. The systems uses an autoencoder for the recognition of vibration patterns from a PC fan. The Ardunio is mounted directly on the fan and measures the data with an accelerometer sensor. The CL system, called TinyOL, is attached after the autoencoder and it is used for fine tuning and recognizing new classes of vibration patterns. The system is able to apply only one CL strategy, which follows the same basic principle of a, ML training, but applied in real time on one sample at a time. 
