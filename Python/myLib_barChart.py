...
def plot_barChart(model):
    ...
    accuracy_bars = np.zeros([len(corr_ary)+1])
    
    for i in range(0, len(corr_ary)):
        accuracy_bars[i] = int(round(corr_ary[i]/tot_ary[i], 2)*100) # Accuracy for each letter
    ...
    # Add to each bar the text with its percentace
    for p in bar_plot:
        height = p.get_height()
        xy_pos = (p.get_x() + p.get_width() / 2, height)
        xy_txt = (0, -20)
